...
u8 RTC_Init(void)
{
   //检查是不是第一次配置时钟
    ...
    if (BKP_ReadBackupRegister(BKP_DR1) != 0x5051)		//从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
    {
        ...
        RTC_Set(2022,10,9,19,43,0); //设置时间
//		RTC_Alarm_Set(2022,10,9,19,34,50);
        RTC_WaitForLastTask();
        RTC_WaitForSynchro();
...
//	calendar.msec=(32767-RTC_GetDivider())*1000/32767;   //毫秒
...
//	
//	//将十进制时间转换成BCD码
//	calendar.hour = (calendar.hour/10)*16+(calendar.hour%10);
//	calendar.min = (calendar.min/10)*16+(calendar.min%10);
//	calendar.sec = (calendar.sec/10)*16+(calendar.sec%10);
//	calendar.w_year = (calendar.w_year/10)*16+(calendar.w_year%10);
//	calendar.w_month = (calendar.w_month/10)*16+(calendar.w_month%10);
//	calendar.w_date = (calendar.w_date/10)*16+(calendar.w_date%10);
